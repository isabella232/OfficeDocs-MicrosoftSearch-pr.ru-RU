...
# <a name="setup-overview-for-graph-connectors-by-microsoft"></a>Обзор установки соединители Graph корпорацией Майкрософт 

В этой статье показан основной процесс, необходимый для создания соединители Graph **корпорацией Майкрософт** в центре администрирования [Microsoft 365.](https://admin.microsoft.com) Основной процесс включает в себя следующие действия:  
<!---Add links to each section in the doc--->

1. [Добавление соединителю Graph в центре администрирования Microsoft 365](#step-1-add-a-graph-connector-in-the-microsoft-365-admin-center)
2. [Имя подключения](#step-2-name-the-connection)
3. [Настройка параметров подключения](#step-3-configure-the-connection-settings)
4. [Управление разрешениями поиска](#step-4-manage-search-permissions)
5. [Назначение меток свойств](#step-5-assign-property-labels)
6. [Управление схемой](#step-6-manage-schema)
7. [Параметры обновления](#step-7-refresh-settings)
8. [Подключение к обзору](#step-8-review-connection)
...
> [!NOTE]
> Процесс установки похож на все соединители Graph от Microsoft, но не совсем то же самое. **В дополнение к чтению этой статьи обязательно ознакомьтесь с информацией, определенной для соединитетеля, для источника данных.**  
...
Выполните следующие действия по настройке любого из встроенных в Microsoft разъемов Graph:

1. Вопишите свою учетную запись администратора в [центре администрирования Microsoft 365.](https://admin.microsoft.com)

2. В области навигации выберите **Параметры,** а затем выберите поиск **&.** Выберите [вкладку Соединители](https://admin.microsoft.com/Adminportal/Home#/MicrosoftSearch/Connectors).

3. Выберите **+Add**, а затем выберите источник данных по вашему выбору из меню доступных вариантов.

   > [!div class="mx-imgBorder"]
   > ![Доступные источники данных: ADLS Gen2, корпоративные веб-сайты, сервер Microsoft SQL, Azure SQL, база данных Oracle SQL, ServiceNow, File Share, Azure DevOps и MediaWiki.](media/add-connector.png)

> [!NOTE]
> Для каждого клиента можно добавить не более десяти подключений Graph.
...
**lastModifiedBy**; | Имя человека, который недавно редактировал элемент
**authors** | Имя людей, которые участвовали или сотрудничали в элементе
**createdDateTime** | Когда был создан элемент
**lastModifiedDateTime** | Когда последний элемент редактировался
...
Атрибуты схемы поиска включают **параметры Запрос,** **Поиск,** **Извлечение** и **уточнение.** В следующей таблице перечислены все атрибуты, поддерживаемые соединиттелями Microsoft Graph, и разъясняются их функции.

Атрибут схемы поиска | Функция | Пример
--- | --- | ---
ПОИСК | Делает текстовое содержимое свойства поиском. Содержимое свойства включено в полно текстовый индекс. | Если свойство является **заголовком,** запрос **для Предприятия** возвращает ответы, содержащие слово **Enterprise** в любом тексте или названии.
ЗАПРОС | Поиск по запросу для совпадения для определенного свойства. Затем имя свойства может быть указано в запросе программным или дословным. |  Если можно запросить свойство **Title,** поддерживается название **запроса: Enterprise.**
RETRIEVE | Только свойства, которые можно получить, можно использовать в типе результата и отобразить в результате поиска. |
УТОЧНЕНИЕ | Параметр уточнения можно использовать как на странице результатов поиска Майкрософт. | Пользователи в организации могут [фильтровать](custom-filters.md) **по URL-адресу** на странице результатов поиска, если свойство уточнения отмечено во время установки подключения.
...
> [!div class="mx-imgBorder"]
> ![Схему соединителем можно настроить, добавив или удалив функции "Запрос", "Поиск" и "Извлечение".](media/manageschema.png)
...
Интервал обновления определяет частоту синхронизации данных между источником данных и microsoft Search. Каждый тип источника данных имеет свой набор оптимальных расписания обновления в зависимости от частоты изменений данных и типа изменений.
...
Сведения об ограничениях, которые применяются к всем источникам данных, см. в статье Обзор соединители [Microsoft Graph.](connectors-overview.md)
