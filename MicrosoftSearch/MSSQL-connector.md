...
## <a name="install-the-graph-connector-agent-required-for-on-premises-microsoft-sql-server-connector-only"></a>Установите агент соединителя Graph (обязательно только для локального соединителя Microsoft SQL Server).
...
## <a name="register-an-app-for-azure-sql-connector-only"></a>Регистрация приложения (только для соединителя Azure SQL)
...
Чтобы подключить соединитель Microsoft SQL Server к источнику данных, необходимо настроить сервер базы данных, для которого выполняется обход, и локальный агент. Затем можно подключиться к базе данных с помощью требуемого метода проверки подлинности.
...
Для обеспечения дополнительной безопасности вы можете настроить правила брандмауэра IP для сервера или базы данных Azure SQL Server. Чтобы узнать больше о настройке правил брандмауэра IP, обратитесь к документации по [правилам IP-брандмауэра](https://docs.microsoft.com/azure/azure-sql/database/firewall-configure). Добавьте в параметры брандмауэра следующие диапазоны IP-адресов клиента.
...
| Точное числовое | Битовая | логический |
| Приблизительные числовые | float <br> режиме реального | double |
