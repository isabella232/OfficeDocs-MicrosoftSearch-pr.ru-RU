...
Макет результатов можно спроектировать для настраиваемого вертикального элемента с помощью конструктора макета поиска. Вы можете начать разработку макета, выбрав шаблоны, предлагаемые в конструкторе макета, и используя их в соответствии с вашими требованиями. Кроме того, вы можете изменить эти шаблоны различными способами в соответствии с вашими требованиями. Например, добавлять и удалять изображения, добавлять и удалять текст, изменять текст. Если ни один из шаблонов не соответствует вашему требованию, вы можете начать разработку макета с помощью пустого шаблона.  

 

После того как макет будет готов, используйте [язык шаблона "адаптивные карты](https://docs.microsoft.com/adaptive-cards/templating/language) ", чтобы создать JSON Layout Layout Layout, который используется для определения типа результата. Свойства результатов сопоставляются с макетом с помощью шага сопоставления в конструкторе макетов.  
...
При создании собственного макета создайте макет адаптивной карточки с помощью данных из, а затем завершите макет.
Создание собственного макета состоит из двух основных этапов.
- Разработка макета
- Отделение данных от шаблона

#### <a name="designing-the-layout"></a>Разработка макета
...
#### <a name="separating-the-data-from-the-layout"></a>Отделение данных от макета
...
- Убедитесь, что типы данных `$when` условия и свойства результата совпадают. Например, не следует сравнивать `Number` с `Text` условием в `$when` .  
- При проектировании структуры результатов следует учитывать требования к теме.  
- Убедитесь, `Textblock`  что элемент может обрабатывать динамическое содержимое. Для этой цели можно `wrap` использовать `maxLines` свойства элемента и. 
...
- Не обрезайте результат на странице результатов, следуя максимальной высоте в формате JSON. Если вы превысили максимальную высоту структуры результатов, результат будет обрезан на странице результатов.
- Не используйте `px` значения в свойствах element.
