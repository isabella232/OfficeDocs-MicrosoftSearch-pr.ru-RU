---
title: ServiceNow Knowledge Graph соединитетелем для Поиск (Майкрософт)
ms.author: kam1
...
- MET150
- MOE150
description: Настройка соединитетеля знаний ServiceNow Graph для Поиск (Майкрософт)
ms.openlocfilehash: 8052571124a74dfa92e5cd81deceee044081ecc1
ms.sourcegitcommit: bb99601a7bd0f16dde7b271de516465d134e5bac
ms.translationtype: MT
ms.contentlocale: ru-RU
ms.lasthandoff: 09/08/2021
ms.locfileid: "58973544"
---
...
С помощью Graph Microsoft для ServiceNow ваша организация может индексировать статьи на основе знаний, которые видны всем пользователям или ограничены разрешениями на использование критериев пользователей в организации. После настройки соединители и индекса контента из ServiceNow конечные пользователи могут искать эти статьи из любого Поиск (Майкрософт) клиента.  

Вы также можете [сослаться на](https://www.youtube.com/watch?v=TVSkJpk1RiE) следующее видео, чтобы узнать больше Graph возможности connector в управлении разрешениями на поиск.

[![Управление разрешениями на поиск в Microsoft Graph Connector для ServiceNow.](https://img.youtube.com/vi/TVSkJpk1RiE/hqdefault.jpg)](https://www.youtube.com/watch?v=TVSkJpk1RiE)

В этой статье Microsoft 365 администраторы или все, кто настраивает, запускает и контролирует соединители serviceNow knowledge Graph. Он дополняет общие инструкции, предусмотренные в статье [Настройка Graph соединители.](configure-connector.md) Если вы еще этого не сделали, ознакомьтесь со всей статьей Настройка Graph Connector, чтобы понять общий процесс настройки.
...
Наряду с этим URL-адресом вам потребуется учетная запись службы для настройки подключения к ServiceNow, а также для Поиск (Майкрософт) периодически обновлять статьи знаний в соответствии с расписанием обновления.  Учетной записи службы потребуется прочитать доступ к следующим записям **таблицы ServiceNow** для успешного обхода различных сущностями.
...
--- | --- | ---
Name | Уникальное значение, которое определяет приложение, для которое требуется доступ к OAuth. | Поиск (Майкрософт)
...
2. В следующей таблице указаны инструкции по заполняемой форме регистрации поставщика OIDC
   ...
   Поле | Описание | Рекомендуемое значение
   --- | --- | ---
   Name | Уникальное имя, идентифицирует сущность OAuth OIDC. | Azure AD
...
В последнем выпуске Graph ServiceNow Knowledge Graph имеет следующие ограничения:
